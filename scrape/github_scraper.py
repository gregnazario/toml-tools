...
def save_results(language: str, results: Dict, filename: str):
    """
    Saves results to a file
    """
    os.makedirs('output', exist_ok=True)
    with open(filename, 'w') as f:
        f.write(f"# {language} Language Repositories\n\n")
...
def compile_and_cleanup_results(language: str, final_filename: str):
    """Combines all batch files into a single file and deletes the batch files."""
    all_results = []
    batch_files = [f for f in os.listdir('output') if f.startswith(f"{language}_repos_batch_")]
...
def process_language(language: str, token: str):
    """
    ...
    repos = list(github_search_repos(f"language:{language}", token))
    ...
    for i in range(0, len(repos), batch_size):
        ...
        logger.info(
            f"Processing batch {i // batch_size + 1} ({i + 1} to {min(i + batch_size, total_repos)} of {total_repos})")

        results = {}
        for owner, repo in batch:
            langs = github_scrape(owner, repo, token)
            if langs and language in langs:
                results[(owner, repo)] = langs[language]
            processed += 1
            ...
            logger.info(
                f"Progress: {processed}/{total_repos} ({processed / total_repos * 100:.1f}%) - Est. remaining time: {remaining / 60:.1f} minutes")

        # Save batch results
        save_results(language, results, f'output/{language}_repos_batch_{i // batch_size + 1}.txt')

    # After all batches are processed, combine them and cleanup
    compile_and_cleanup_results(language, f"output/{language}_repos_final.txt")
...
def main():
    ...
    languages = ["JavaScript", "TypeScript", "Go", "Rust", "Move", "Python", "Java", "Swift", "Kotlin"]

    for language in languages:
        logger.info(f"Processing {language}")
        process_language(language, token)
